...
// $Id: crfsgd.cpp,v 1.35 2007/12/07 16:47:50 cvs Exp $
...
usage()
{
  cerr 
    ...
    << "Usage (converting model into an interpretable text)" << endl
    << "crfsgd -v model output" << endl
  ...
  for (int i=1; i<argc; i++)
    {
      ...
      if (s[0]=='-')
        ...
        {
          if (modelToTextFile.empty())
            modelToTextFile = argv[i];
          else
            usage();
        }
      else
...
main(int argc, char **argv)
{
  ...
  if (modelToText == true)
  {
      cout << "[Log] Converting model to interpretable text file." << endl;
      cout << "[Log] Model file: " << modelToTextFile << endl;
      return 0;
  }
